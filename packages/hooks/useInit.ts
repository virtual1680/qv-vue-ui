...
export const useInitCrud = (props: any) => {
	...
	watch(
		() => props.option,
		() => {
			// console.log('传进来的option有变化--------watch---21---', props.option)
			init(false)
		},
		{
			deep: true,
	...
	// 	() => props.option.column,
	// 	() => {
	// 		// console.log('传进来的option有变化--------watch---22---', props.option)
	// 		init(false)
	// 	},
	// 	{
	// 		deep: true,
	...
	onMounted(() => {
		init()
		// console.log('传进来的option有变化--------mounted------', props.option)
	})
	return {
		tableOption,
		DIC,
