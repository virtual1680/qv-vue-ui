import { createObj, detailDataType, findObject, validatenull } from '@qv-vue/utils'
import {
	ARRAY_LIST,
	ARRAY_VALUE_LIST,
	DATE_LIST,
...
export const initValue = (value: any, column: any) => {
	const { type, multiple, dataType, separator = DIC_SPLIT, alone, emitPath, range } = column
	let list = value
	if (
...
export const formInitVal = (list: any[] = []) => {
	...
	list.forEach((ele: any) => {
		if (ARRAY_VALUE_LIST.includes(ele.type) || (MULTIPLE_LIST.includes(ele.type) && ele.multiple) || ele.dataType === 'array') {
			tableForm[ele.prop] = []
