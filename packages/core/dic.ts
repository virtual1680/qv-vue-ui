...
export const loadLocalDic = (option: { dicData: Record<string, unknown>; column: any[] }) => {
	const locationdic: Record<string, unknown> = {}
	const alldic = option.dicData || {}
	option.column.forEach((ele: { dicData: any[] | undefined; prop: string | number; props: any; dataType: string }) => {
...
function createdDic(option: { column: never[] }) {
	...
	column.forEach(
		(ele: {
			...
			const result = ele.dicFlag === false || ele.lazy === true || flagdic.includes(prop)
			if (result) return
...
export const sendDic = (params: any) => {
	const innerInstance = getCurrentInstance()
	let { url, query, method, resKey, props, formatter, value = '', column, form = {}, instance } = params
	if (column) {
		url = column.dicUrl
		method = column.dicMethod
		query = column.dicQuery || {}
	...
	return new Promise(resolve => {
		...
		const composeInstance = innerInstance || instance
		// console.log(composeInstance)
