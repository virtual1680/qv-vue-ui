                ...
                >
                  ...
                  <template v-if="getSlotName(column, 'E')" #error="scope">
                    <slot
                      ...
                      v-bind="formItemBind(scope, column)"
                    />
                  </template>
                  <component
...
const columnSlot = computed(() => {
  return Object.keys(slots).filter(
    (item) => !propOption.value.map((ele) => ele.prop).includes(item)
  )
})
        ...
        const callback = () => {
          ...
          Object.keys(controlList).forEach((item) => {
            ...
            if (controlList[item].dicData)
              DIC.value[item] = controlList[item].dicData
          })
        }
    ...
    cascader.forEach((item: any) => {
      ...
      sendDic({
        ...
        if (
          ...
          validatenull(form.value[columnNextProp])
        ) {
          form.value[columnNextProp] =
...
const validate = async (
  callback: (validate: boolean, hide: () => void, result?: any) => void
) => {
    ...
    dynamicOption.value.forEach((ele: any) => {
      ...
      if (isForm) {
        if (!validatenull(propRef.value[ele.prop]?.tempRef.value.$refs.main)) {
          propRef.value[ele.prop].tempRef.value.$refs.main.forEach(
            (ele: any) => {
        ...
        dynamicList.push(
          propRef.value[ele.prop].tempRef.value.$refs.main.validateCellForm()
        )
      }
    })
...
const resetForm = () => {
  form.value = clearVal(
    form.value,
    (tableOption.value.clearExclude || []).concat([rowKey.value])
  )
  nextTick(() => {
    clearValidate()
    emit('reset-change')
