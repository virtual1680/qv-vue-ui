<template>
	<el-select
		...
		<template v-if="isGroup">
			<el-option-group v-for="(item, index) in netDic" :key="index" :label="getLabelText(item)">
				<el-option
					...
					<template #default>
						<slot v-if="slots.default" :label="labelKey" :value="valueKey" :item="citem" />
						<template v-else>
							<span>{{ getLabelText(citem) }}</span>
							<span v-if="citem[descKey]" :class="b('desc')">{{ citem[descKey] }}</span>
						</template>
					</template>
				</el-option>
			</el-option-group>
		...
		<template v-else>
			<el-checkbox v-if="column?.all" v-model="checkAll" :class="b('check')" @change="selectAll"> 全选 </el-checkbox>
			<el-option v-for="(item, index) in netDic" :key="index" :disabled="item[disabledKey]" :label="getLabelText(item)" :value="item[valueKey]">
				<template #default>
					<slot v-if="slots.default" :label="labelKey" :value="valueKey" :item="item" />
					<template v-else>
						<span>{{ getLabelText(item) }}</span>
						<span v-if="item[descKey]" :class="b('desc')">{{ item[descKey] }}</span>
					</template>
				</template>
			</el-option>
		</template>
...
const handleModelValue = () => {
	if (!validatenull(text.value)) {
		if (props.column?.remote && !created.value) {
			created.value = true
			handleRemoteMethod(props.column?.multiple ? text.value.join(DIC_SPLIT) : text.value)
		}
	}
}

let { text, handleFocus, handleBlur, isGroup, labelKey, groupsKey, descKey, handleClick, clearableVal, valueKey, disabledKey, getLabelText } =
