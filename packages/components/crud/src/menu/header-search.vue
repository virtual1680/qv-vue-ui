<template>
  <el-card v-show="searchShow && searchFlag" :shadow="crud.isCard" :class="b()">
    ...
    >
      ...
      <template
        v-for="item in crud.searchSlot.value"
...
const isSearchIcon = computed(() => {
  return (
    validData(crud.option.searchIcon, true) === true &&
    columnLen.value > searchIndex.value
  )
})
...
const option = computed<QvOption>(() => {
    ...
    list.forEach((ele: QvColumn) => {
      if (ele.search) {
        ...
        ele = Object.assign(ele, obj, {
          ...
          gutter:
            ele.searchGutter || cOption.searchGutter || config.searchGutter,
          labelWidth:
            ele.searchLabelWidth ||
            cOption.searchLabelWidth ||
  ...
  const dataDetail = (option: QvOption) => {
    ...
    result.column =
      crud.propOption.value.length > 0
        ? detailColumn(deepClone(crud.propOption.value))
        : []
