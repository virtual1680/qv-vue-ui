...
const list = computed(() => {
  ...
  result = arraySort(
    ...
    'index',
    (a: any, b: any) =>
      crud.objectOption.value[a.prop]?.index -
      crud.objectOption.value[b.prop]?.index
  )
...
const handleFiltersMethod = (value: any, row: any, column: any) => {
  const columnNew: QvColumn | undefined = props.columnList.find(
...
const getColumnProp = (
  column: QvColumn,
  type: 'filterMethod' | 'fixed' | 'width' | 'filters' | 'hide'
) => {
