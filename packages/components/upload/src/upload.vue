  ...
  <div v-loading.lock="loading" :class="b()">
    ...
    >
      ...
      <template #file="{ file }">
        ...
        <span
          v-else-if="column?.listType === 'picture'"
  ...
  ;(text.value || []).forEach((ele: any, index: number) => {
    if (ele) {
      ...
      const url = getFileUrl(
        homeUrl.value,
        isMultiple.value ? ele : ele[valueKey.value]
      )
      list.push({
        uid: index,
        status: 'done',
...
const deleteImg = (file: any) => {
  ;(text.value || []).forEach((ele: any, index: number) => {
    if (
      (isMultiple.value ? ele : ele[valueKey.value]) ===
      file.url.replace(homeUrl.value, '')
    ) {
    ...
    const callback = (newFile?: any) => {
      ...
      ;(() => {
        ...
        .then((result: any) => {
          res.value = {}
          if (isQiniuOss.value) {
            result.data.key = ossConfig.url + result.data.key
          }
...
const handleExceed = (files: any, fileList: any[]) => {
  props.column?.uploadExceed &&
    props.column?.uploadExceed(
      props.column?.limit || 1,
      files,
      fileList,
      props.column
    )
